...
describe('unified inbox cards and review workspace', () => {
  ...
  it('opens any filtered item on the existing human-review workspace without sending', async () => {
    await withKfzEnv(async () => {
      ...
      assert.equal(workspace.sections.history, true)
      assert.equal(workspace.sections.task, true)
      assert.equal(workspace.history.events[0]?.kind, 'received')
      assert.equal(workspace.history.includesAiSuggestion, false)
      assert.match(workspace.historyHref, /view=history/)
      assert.doesNotMatch(workspace.workHref, /view=history/)
